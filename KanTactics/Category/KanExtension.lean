...
structure LeftKanExtension (K : Functor C D) (F : Functor C E) where
...
structure RightKanExtension (K : Functor C D) (F : Functor C E) where
...
theorem lan_along_identity (F : Functor C E) :
    ∃ (_ : LeftKanExtension (Functor.identity) F),
      True := by
  exact ⟨{
    lan := F
